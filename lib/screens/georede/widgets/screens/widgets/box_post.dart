...
class BoxPost extends StatefulWidget {
  ...
  @override
  State<BoxPost> createState() => _BoxPostState();
}
...
class _BoxPostState extends State<BoxPost> {
  ...
  Widget build(BuildContext context) {
    String text = '';
    CollectionReference users = FirebaseFirestore.instance.collection('redePA');
    final user = FirebaseAuth.instance.currentUser;
    Future<void> addUser() {
      return users
          .add({
            'like': 0, // John Doe
            'name': user!.displayName, // Stokes and Sons
            'photo': user.photoURL,
            'text': text,
          })
          .then((value) => Navigator.of(context).pushNamed(AppRoutes.georedeA))
